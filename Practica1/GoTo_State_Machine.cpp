...
int go_to(Inputs inputs)
{
	...
	while (flg == 0)
	{
		...
		{
			// It calculates the robot's movement using an state machine created by an student RODEAR
		...
		{
			// It calculates the robot's movement using an state machine created by an student BUG 1
			state = next_state;
			// state_machine_students in ../state_machines/state_machine_student.h
			DistTheta = BUG1(coord_robot, coord_dest, observations, quantized_attraction, quantized_intensity, state, &next_state, inputs.Mag_Advance, inputs.max_angle, inputs.num_sensors);
